...
import { createPremiumOrb } from "./orbVisuals";
...
export interface OrbSceneApi {
  ...
  resetView(): void;
  dispose(): void;
  /** Audio-reactive: 0..1 level makes the core mouth/pulse while speaking. */
  setAudioLevel(level: number): void;
  /** Set orb mood/state label shown by the agent (listening/thinking/speaking/idle). */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // ═══════════════════════════════════════════════
  // PREMIUM ENERGY ORB (cinematic core + nebula + god-rays)
  // ═══════════════════════════════════════════════
  const premium = createPremiumOrb();
  orbGroup.add(premium.group);
  ...
  // ULTRON audio-reactive + state hooks
  let audioLevel = 0;        // 0..1 smoothed from setAudioLevel
  let targetAudio = 0;
  ...
  function animate() {
    ...
    if (orbState === "speaking") bloom.strength += audioLevel * 1.2;

    // Audio-reactive core surge (ULTRON mouthing)
    targetAudio = THREE.MathUtils.lerp(targetAudio, audioLevel, 0.3);
    const mouthSurge = targetAudio * 1.8;
    const coreSurge = surge + mouthSurge;
    coreSphere.scale.setScalar(coreScale + mouthSurge * 0.4);
    coreSphereMat.opacity = Math.min(0.85, coreOpacity + targetAudio * 0.5);
    glowSphereMat.opacity = Math.max(0, (0.03 + surge * 0.08 + targetAudio * 0.3) * (1 - fadeOut * 0.9));
    icoWire.scale.setScalar(1 + coreSurge * 0.6);
    icoWireMat.opacity = Math.min(1, 0.5 + coreSurge * 0.4 + (orbState === "listening" ? 0.2 : 0));

    // Update chromatic aberration time
    chromaticPass.uniforms.uTime.value = t;

    // Premium cinematic orb
    premium.update(t);
  ...
  return {
    ...
    setAudioLevel(level: number) {
      ...
      premium.setAudioLevel(audioLevel);
    },
    setState(state: "idle" | "listening" | "thinking" | "speaking") {
      orbState = state;
      premium.setState(state);
    },
  };
}
